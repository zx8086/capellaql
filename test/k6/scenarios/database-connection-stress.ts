  ...
  try {
    ...
    const connectionHealth = check(response, {
      'no connection timeout errors': (r) => {
        ...
        return !body.includes('connection timeout') && 
    ...
    if (!connectionHealth || response.status !== 200) {
      ...
      if (response.status === 0 || response.timings.duration > 5000) {
        console.warn(`Potential connection pool exhaustion - Operation: ${operation}, Status: ${response.status}, Duration: ${response.timings.duration}ms`);
      }
    }

    // Simulate connection cleanup delay
    if (Math.random() < 0.1) { // 10% chance of slow cleanup
