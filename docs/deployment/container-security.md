...
|---------|-------------|------|
| SLSA Level 3 | Verifiable build process | Docker BuildKit |
| SBOM Generation | Software Bill of Materials (syft-json) | Docker BuildKit |
| Image Signing | Cryptographic signature on pushed images | Cosign |
| SBOM Attestation | Signed attestation linking SBOM to image | Cosign |
| VEX | Vulnerability exploitability analysis | Docker Scout |
| Provenance | Build attestation | Docker BuildKit |
| CVE Scanning | Vulnerability detection | Trivy, Snyk |
| Code Analysis | Semantic security analysis | CodeQL |

### Image Signing with Cosign

CapellaQL container images are signed using [Cosign](https://docs.sigstore.dev/cosign/overview/) as part of the CI/CD pipeline. Signing occurs automatically on push to `master` (not on pull requests).
...
# Verify a specific digest
cosign verify \
  --key cosign.pub \
  docker.io/zx8086/capellaql@sha256:<digest>
```
...
# Store the private key as a GitHub secret (COSIGN_PRIVATE_KEY)
# Store the password as a GitHub secret (COSIGN_PASSWORD)
# Store cosign.pub in the repository root for verification
```
...
| `DOCKER_HUB_USERNAME` | Docker Hub username for push |
| `DOCKER_HUB_ACCESS_TOKEN` | Docker Hub access token |

#### VEX Attestations

For known false-positive CVEs, create VEX (Vulnerability Exploitability eXchange) statements:

```bash
# Create a VEX attestation for a known false positive
cosign attest --yes \
  --key cosign.key \
  --type vuln \
  --predicate vex.json \
...
#### CI/CD Pipeline Security Phases

The Docker CI/CD pipeline implements defense-in-depth:

1. **Build Phase**: Multi-stage builds with DHI distroless base image, non-root user (UID 65532)
2. **Sign Phase**: Cosign signing with SBOM attestation (push only, skipped on PRs)
3. **Scan Phase**: Parallel 3-scanner security matrix (Trivy, CodeQL, Snyk)
