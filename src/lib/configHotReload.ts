...
import { configWatcher, getConfigDifferences, parseEnvContent, readConfigFile } from "./configWatcher";
...
class ConfigurationHotReload extends EventEmitter {
  ...
  private async validateConfiguration(envVars: Record<string, string>): Promise<ConfigurationValidationResult> {
    ...
    try {
      ...
      const requiredVars = ["COUCHBASE_URL", "COUCHBASE_USERNAME", "COUCHBASE_BUCKET"];
      ...
      // Validate URL format
      if (
        envVars.COUCHBASE_URL &&
        !envVars.COUCHBASE_URL.startsWith("couchbase://") &&
        !envVars.COUCHBASE_URL.startsWith("couchbases://")
      ) {
