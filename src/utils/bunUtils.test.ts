...
describe("BunUtils - Core Functionality", () => {
  ...
  describe("BunProcess management", () => {
    ...
    test("should handle command with environment variables", async () => {
      const result = await BunProcess.spawn(["node", "-e", "console.log(process.env.TEST_VAR)"], {
    ...
    test("should handle timeouts", async () => {
      ...
      await expect(BunProcess.spawn(["sleep", "5"], { timeout: 100 })).rejects.toThrow();
...
describe("BunUtils - Resilience Patterns", () => {
  describe("Circuit Breaker", () => {
    ...
    test("should open after threshold failures", async () => {
      ...
      await expect(circuitBreaker.execute(async () => "should not execute")).rejects.toThrow("Circuit breaker is open");
    });
  ...
  describe("Retry with Backoff", () => {
    ...
    test("should fail after max retries", async () => {
      ...
      await expect(retryWithBackoff(mockOperation, 2, 10)).rejects.toThrow("Persistent failure");
...
describe("BunUtils - Error Handling", () => {
  test("should handle file operation errors gracefully", async () => {
    await expect(BunFile.read("/nonexistent/path/file.txt")).rejects.toThrow();

    await expect(BunFile.readJSON("/nonexistent/config.json")).rejects.toThrow();
  });

  test("should handle process execution errors", async () => {
    await expect(BunProcess.spawn(["nonexistent-command"])).rejects.toThrow();
  });
  ...
  test("should handle malformed JSON gracefully", async () => {
    ...
    await expect(BunFile.readJSON(testFile)).rejects.toThrow();
  });
});
