...
var env = typeof Bun < "u" ? Bun.env : typeof process < "u" ? process?.env : void 0, ERROR_CODE = Symbol("ElysiaErrorCode"), isProduction = (env?.NODE_ENV ?? env?.ENV) === "production", emptyHttpStatus = {
  ...
  constructor(type, validator, value, errors) {
    ...
    let error2 = errors?.First() ?? ("Errors" in validator ? validator.Errors(value).First() : import_value.Value.Errors(validator, value).First()), accessor = error2?.path || "root", schema = validator?.schema ?? validator, expected;
    if (!isProduction)
      try {
    ...
    customError !== void 0 ? message = typeof customError == "object" ? JSON.stringify(customError) : customError + "" : isProduction ? message = JSON.stringify({
    ...
    }) : message = JSON.stringify(
      {
        ...
        expected,
        found: value,
        errors: "Errors" in validator ? [...validator.Errors(value)].map(mapValueError) : [...import_value.Value.Errors(validator, value)].map(
          mapValueError
        )
      },
      null,
